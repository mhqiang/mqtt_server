...
func (e EntryInfo) Value() interface{} {
	return e.value
}
...
func (it *EntryInfoIterator) SetNext() bool {
	...
	for i := it.currentShard + 1; i < it.cache.config.Shards; i++ {
		it.elements, _, _, it.elementsCount = it.cache.shards[i].copyKeys()
...
func newIterator(cache *BigCache) *EntryInfoIterator {
	elements, keys, hashs, count := cache.shards[0].copyKeys()
...
func (it *EntryInfoIterator) Value() (EntryInfo, error) {
	...
	curShard := it.cache.shards[it.currentShard]
	entry := it.elements[it.currentIndex]
	key := it.keys[it.currentIndex]
	hash := it.hashs[it.currentIndex]
	if entry == nil {
		it.mutex.Unlock()
	...
	return EntryInfo{
		timestamp: curShard.onExpire(entry),
		hash:      hash,
		key:       key,
		value:     entry,
	}, nil
