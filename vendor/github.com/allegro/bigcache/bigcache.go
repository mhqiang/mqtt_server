...
func NewBigCache(config Config, eleCallback func(src, cur interface{}) bool,
	expireCallback func(value interface{}) uint64, expandqueue func(size int) (*utils.List, error),
	setData func(src, cur interface{})) (*BigCache, error) {
	return newBigCache(config, &systemClock{}, eleCallback, expireCallback, expandqueue, setData)
}

func newBigCache(config Config, clock clock, eleCallback func(src, cur interface{}) bool,
	expireCallback func(value interface{}) uint64, expandqueue func(size int) (*utils.List, error),
	setData func(src, cur interface{})) (*BigCache, error) {
	...
	for i := 0; i < config.Shards; i++ {
		cache.shards[i] = initNewShard(config, onRemove, clock, eleCallback, expireCallback,
			expandqueue, setData)
	}
...
func (c *BigCache) Set(key string, entry interface{}) error {
	hashedKey := c.hash.Sum64(key)
	shard := c.getShard(hashedKey)
...
// Set saves entry under the key
func (c *BigCache) Update(key string, entry interface{}) (error, bool) {
	hashedKey := c.hash.Sum64(key)
	shard := c.getShard(hashedKey)
	return shard.update(key, hashedKey, entry)
}
...
func (c *BigCache) cleanUp(currentTimestamp uint64) {
	// 设置goroutine 去cleanup 可能会占用更多cpu，并会同时锁住所有得list，
	// 导致程序锁死
	for _, shard := range c.shards {
